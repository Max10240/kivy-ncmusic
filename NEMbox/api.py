...
class Queue(queue.Queue):
    ...
    def get(self,key):
        v=queue.Queue.get(self)
        if v[0] != key:
            logger.warning('Bad value: %s '%v)
            return self.get(key)
...
##        try:
##            os.makedirs('.netease-musicbox')
##        except FileExistsError:pass
        
        if os.path.exists(self.storage.storage_path):
            self.storage.load()
        
        if not os.path.exists(self.storage.cookie_path):
            with open(self.storage.cookie_path,'w')as f:
...
##        try:
##            if not daily_task['message']:
##                q_put.put([None,'input_un_pd'])
##        except:pass
    ...
    def login(self, username, password, need_md5_enc=False):
        if need_md5_enc:password=Encrypt.md5(password)
    ...
    def user_playlist(self, uid=None, offset=0, limit=50):
        uid = uid or self.storage.database['user']['user_id']
    ...
    def songs_url(self, ids, quality=2):
        if type(ids) != list:ids=list(ids)
        
##        quality = quality or Config().get("music_quality")
